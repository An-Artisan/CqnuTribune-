<?php
   ...
   * @参数:      
    ...
    private $maxsize = 10485760;           //限制文件上传大小（字节）这里设置的是10M
   ...
   * @方法     
          set()-设置文件上传路径，设置图片的最大字节，设置允许上传类型等
          upload()-上传文件
          getFileName()-获取文件名
          getErrorMsg()-获取错误提示
          .
          ...
          .
          自己往下看
   * @功能:   本类的实例对象用于处理上传文件，可以上传一个文件，也可同时处理多个文件上传
   * @作者:   不敢为天下
  */
  class FileUpload { 
    private $path = "./uploads";          //上传文件保存的路径
     ...
     * 可以通过连贯操作一次设置多个属性值
     * string $key  成员属性名(不区分大小写)
     * mixed  $val  为成员属性设置的值
     * object     返回自己对象$this，可以用于连贯操作
     */
