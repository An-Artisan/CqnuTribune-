...
function navigation_check_sort(sorf){
...
$("#navigation_submit").click(function(){
    $.ajax({  
         type : "post",  
          url : "../backStageBackStage/addNavigation.php", 
          dataType:'json',
          data : {
                navigation_name: $('#navigation_name').val(),
                navigation_alias: $('#navigation_alias').val(),
                navigation_url: $('#navigation_url').val(),
                navigation_sort: $('#navigation_sort').val()
                // 传后台的用户编号
            },
          async : true,  
          // 是否异步，默认是true
