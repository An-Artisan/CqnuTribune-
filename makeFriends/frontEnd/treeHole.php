            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul id="ul1" class="nav navbar-nav">
                    <?php 
                        ...
                        // 获取查询对象
                        $sql_content_is_read = "update makefriend.tree_publish set p_content_is_read = 1 where p_number = (select h_id from makefriend.treehole where h_name = '".$_SESSION['user']."' and h_id = ".$_GET['p_number'].") and p_getter = '' ";
                        // 修改未读信息为已读信息
                        $select->query($sql_content_is_read);
                        $sql_each_content_is_read = "update makefriend.tree_publish set p_content_is_read = 1 where p_getter = '". $_SESSION['user'] . "' and p_number = '" . $_GET['p_number'] ."'";
                        $select->query($sql_each_content_is_read);
                        // 修改互相回复的未读消息为已读消息
            ...
            <div class="">
                ...
                <div id="list" class="container-fluid container-fluid1">
                 <?php 
                    ...
                    $sql = "select t.h_id,t.h_content,t.h_name,t.h_praise,t.h_time ,u.user_photo from makefriend.treehole t  left join user.user_information u on t.h_name = u.user_name where h_id = " . $p_number;
                    // 查询所有的导航信息
                    $result = $select->select($sql);
                    // 获取结果集
